function bybit_DatabaseUpdater_() {
  key = bybit_key;
  secret = bybit_secret;
  if (key == "" || secret == "") {
  ...
  //取引履歴を参照
  var [productcodes, tids, times] = bybit_getTable_();
  try {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var tid = tids[i];
    var time = times[i];
    var result = bybit_getExecutions_(productcode, tid, time);
    if (result) {
      ids.push(result[0]);
  ...
  //Databaseの価格を更新する
  bybit_updateTable_(tids2, price2, exec_date2);
}

function bybit_getTable_() {
  var productcodes = []
  var tids = [];
  var times = [];
  var result = [];
...
function bybit_getExecutions_(productcode, order_id, time) {
  key = bybit_key;
  secret = bybit_secret;
...
function bybit_updateTable_(tids, prices, exec_dates) {
